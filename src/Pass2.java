...
public class Pass2 {
    ...
    static String computeObjectCode(String loc, String label, String op, String operand) {
        ...
        String opcodeHex = opcodeTable.get(op.toUpperCase());
        if (opcodeHex == null) {
        ...
        if (isFormat3F) {
            int opVal = Integer.parseInt(opcodeHex, 16) & 0xFC; // Ensure 6-bit opcode
            int f1 = (disp % 2 == 0) ? 1 : 0;
            int f2 = (disp >= 0) ? 0 : 1;
            int f3 = (disp == 0 && b == 1) ? 1 : 0;

            // Correct bit positioning: Opcode remains untouched, flags correctly placed
            int firstByte = opVal;
            int secondByte = (f1 << 7) | (f2 << 6) | (x << 5) | (b << 4) | (p << 3) | (f3 << 2) | ((disp >> 8) & 0x0F);
